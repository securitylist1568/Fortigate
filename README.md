...
Kali Linux (自動化腳本)
        ...
        └─ ncloud_topn_feed_push_v4_2.py        ← TopN 威脅情資推送器
                │
...
| `ncloud_block_src_topn.txt` | 流量分析產生之每日 Top-N 威脅來源 IP | `ncloud_topn_feed_push_v4_2.py` 自動生成 | ~3,437 |
| `ncloud_block_src_trojan_irc.txt` | 連線至 Trojan / IRC C2 伺服器之惡意來源 IP | `ncloud_topn_feed_push_v4_2.py` 自動生成 | ~1,561 |
| `ncloud_block_src_cert_remote.txt` | 使用可疑憑證進行遠端連線之來源 IP | `ncloud_topn_feed_push_v4_2.py` 自動生成 | ~1,099 |
| `ncloud_block_src_ext_ssh_in.txt` | 外部主機對內 SSH 入侵嘗試之來源 IP | `ncloud_topn_feed_push_v4_2.py` 自動生成 | ~801 |
| `ncloud_block_src_dns_ptr_anom.txt` | DNS PTR 記錄異常之可疑來源 IP | `ncloud_topn_feed_push_v4_2.py` 自動生成 | ~180 |
| `ncloud_block_dst_topn.txt` | 流量分析產生之 Top-N 威脅目標 IP | `ncloud_topn_feed_push_v4_2.py` 自動生成 | 動態（視威脅情況） |
...
| 腳本名稱 | 主要功能 | 更新的清單 |
|---|---|---|
| `ncloud_topn_feed_push_v4_2.py` | 每日重算 + 每 10 分鐘增量更新，分析流量威脅 Job（`ext_to_int_threat_unblocked`、`int_to_ext_flow_ioc`、`trojan_irc_top`、`cert_remote_top`、`dns_ptr_anomaly`、`ext_to_int_ssh_inflow`） | `ncloud_block_src_topn.txt`、`ncloud_block_src_trojan_irc.txt`、`ncloud_block_src_cert_remote.txt`、`ncloud_block_src_ext_ssh_in.txt`、`ncloud_block_src_dns_ptr_anom.txt`、`ncloud_block_dst_topn.txt` |
...
# Generated by ncloud_topn_feed_push_v4_2.py
